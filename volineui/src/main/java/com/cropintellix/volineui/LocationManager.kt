...
class LocationManager private constructor(
     ...
     * 
     * @param callback Callback invoked with cached location (null if unavailable)
     */
    @Suppress("MissingPermission")
    fun getCachedLocation(callback: (LocationResult?) -> Unit) {
     ...
     * activate GPS/network location providers. If permission is not granted,
     * returns null. Falls back to cached location if fresh location unavailable.
     * 
     * @param timeout Timeout in milliseconds (default: 30 seconds)
     * @param callback Callback invoked with location result (null if unavailable)
     */
    @Suppress("MissingPermission")
    ...
    /**
     * Request location permission via PermissionManager
     * 
     * Convenience method to request location permission.
     * Requires PermissionManager to be initialized.
     * 
     * @param callback Callback with permission result
     */
    fun requestLocationPermission(callback: (Map<String, PermissionResult>) -> Unit) {
        try {
            PermissionManager.instance.requestLocationPermission(callback)
        } catch (e: PermissionException) {
            throw LocationException(
                "PermissionManager not initialized. Call PermissionManager.init() before using LocationManager."
            )
        }
    }
